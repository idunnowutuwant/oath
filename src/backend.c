        ...
        switch (stmt->type) {
            case STMT_MATCH: {
                ...
                for (size_t a = 0; a < stmt->as.match_stmt.arm_count; ++a) {
                    ...
                    fprintf(out, "case %zu: { // %s\n", v_idx, ed->variants[v_idx].name);
...
static void emit_param_type_c(FILE* stream, const OathFunction* fn, size_t p, const OathModule* mod) {
    ...
    } else if (param->kind == PARAM_BUFFER || param->kind == PARAM_SLICE) {
...
void oath_emit_c_header(FILE* stream, const OathModule* mod, const char* prefix) {
    ...
    fprintf(stream, "/**\n");
    fprintf(stream, " * @file %s.h\n", prefix);
    fprintf(stream, " * @brief Formally Verified Module generated by OATH Compiler Core\n");
    fprintf(stream, " * @note 100%% Mathematically Sound: No Div-By-Zero, No Buffer Overflow, No Traps\n");
    fprintf(stream, " */\n\n");
    ...
    for (size_t s = 0; s < mod->struct_count; ++s) {
        const OathStructDef* sd = &mod->structs[s];
        fprintf(stream, "/**\n");
        fprintf(stream, " * @brief Verified Struct: %s\n", sd->name);
        for (size_t f = 0; f < sd->field_count; ++f) {
            fprintf(stream, " * @invariant %s Range [%" PRId64 ", %" PRId64 "]\n",
                    sd->fields[f].name, sd->fields[f].bounds.lo, sd->fields[f].bounds.hi);
        }
        fprintf(stream, " */\n");
        fprintf(stream, "typedef struct {\n");
        for (size_t f = 0; f < sd->field_count; ++f) {
    ...
    for (size_t i = 0; i < mod->function_count; ++i) {
        const OathFunction* fn = &mod->functions[i];
        fprintf(stream, "/**\n");
        fprintf(stream, " * @brief Target: %s%s\n", fn->name, fn->is_extern ? " (CVI Extern)" : "");
        for (size_t d = 0; d < fn->disjoint_count; ++d) {
            fprintf(stream, " * @note Memory Separation: '%s' and '%s' are formally proven non-overlapping (restrict)\n",
                    fn->params[fn->disjoint_contracts[d].param_a].name,
                    fn->params[fn->disjoint_contracts[d].param_b].name);
        }
        for (size_t p = 0; p < fn->param_count; ++p) {
            if (fn->params[p].kind == PARAM_STRUCT) {
                fprintf(stream, " * @param %s Struct of type %s\n", fn->params[p].name, mod->structs[fn->params[p].struct_def_idx].name);
            } else if (fn->params[p].kind == PARAM_ENUM) {
                fprintf(stream, " * @param %s Enum of type %s\n", fn->params[p].name, mod->enums[fn->params[p].enum_def_idx].name);
            } else if (fn->params[p].kind == PARAM_BUFFER) {
                fprintf(stream, " * @param %s Array pointer [capacity: %zu, elem_bounds: [%" PRId64 ", %" PRId64 "]]\n",
                        fn->params[p].name, fn->params[p].buffer.capacity,
                        fn->params[p].buffer.elem_bounds.lo, fn->params[p].buffer.elem_bounds.hi);
            } else if (fn->params[p].kind == PARAM_SLICE) {
                fprintf(stream, " * @param %s Dynamic slice [bounded by: %s, elem_bounds: [%" PRId64 ", %" PRId64 "]]\n",
                        fn->params[p].name, fn->params[fn->params[p].slice.len_param_idx].name,
                        fn->params[p].slice.elem_bounds.lo, fn->params[p].slice.elem_bounds.hi);
            } else {
                fprintf(stream, " * @param %s Required interval [%" PRId64 ", %" PRId64 "]\n",
                        fn->params[p].name, fn->params[p].scalar_bounds.lo, fn->params[p].scalar_bounds.hi);
            }
        }
        fprintf(stream, " * @return Guaranteed contract bounds [%" PRId64 ", %" PRId64 "]\n",
...
void oath_emit_b2b_certificate(FILE* stream, const OathFunction* fn, const SepeReport* report) {
    ...
    fprintf(stream, "{\n");
    fprintf(stream, "  \"schema\": \"OATH-FINAL-v1.0\",\n");
